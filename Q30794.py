...
가희는 클럽 오디션을 하고 있습니다. 이 게임에는 여러 모드가 있는데 가희는 8키 모드를 하고 있습니다. 8키 모드에서는 레벨이 6 이상인 키 노트가 등장합니다. 레벨이 
$lv$인 키 노트 안에는 
$lv$개의 키가 있습니다. 키의 방향은 8 방향 중 (상, 하, 좌, 우, 좌상, 좌하, 우상, 우하) 하나입니다.
...
$n$연팩. 
$n$번 연속 perfect 판정이 난 상태를 
$n$연팩이라 합니다. 발생한 키 노트의 판정이 perfect일 때

이전 판정이 perfect가 아닌 경우 
$1$연팩입니다.
이전 판정이 perfect이고 
$k$연팩 상태였다면 
$k+1$연팩이 됩니다.
레벨이 
$lv$인 키 노트의 판정에 따른 점수 획득은 아래와 같습니다.
...
$1\,000$을 곱한 점수를 받습니다.
가희가 친 키 노트의 레벨 
$lv$과 판정이 주어집니다. 가희가 이전에 받은 판정과 다른 판정을 받았을 때 획득한 점수를 구해주세요.

입력
첫 줄에 키 노트의 
$lv$과 판정이 공백으로 구분되어 주어집니다. 이때 판정은 miss, bad, cool, great, perfect중 하나로 주어집니다.

출력
가희가 획득한 점수를 출력해 주세요.
"""

lv, decision = map(str, input().split())
