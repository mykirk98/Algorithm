...
현재 good-gum알고리즘을 통해 사용자에게 총 $N$개의 콘텐츠가 추천된 상태이다.
이 각 콘텐츠 별로 계산된 흥미도 값과 해당 콘텐츠가 이미 My뷰에 등록되어 있는지 여부가 주어졌을 때, 전체 흥미도의 합과 My뷰에 등록되지 않은 콘텐츠의 흥미도의 합을 각각 구해서 큐레이팅 알고리즘이 얼마나 유용한지를 분석해보자.
...
alreadyHave = list(map(int, input().split()))
...
for n in range(N):
    if alreadyHave[n] == 0:
        sum_notHaveYet += myView[n]
print(sum_notHaveYet)